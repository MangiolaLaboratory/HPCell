...
metadata =
  data_for_pca |>
  pivot_sample() |>
  dplyr::select(Tissue, .sample, alive, Cell_type_in_each_tissue, .aggregated_cells)
...
x<- ggplot(my_pca$metadata, aes(x = my_pca$rotated[, "PC1"], y = my_pca$rotated[, "PC2"], color = my_pca$metadata$Tissue))  +
  geom_point() +
  theme_minimal() +
...
ggplot(aes(PC1, PC2, color=Cell_type_in_each_tissue)) +
geom_point() +
  theme_bw() +
  theme(

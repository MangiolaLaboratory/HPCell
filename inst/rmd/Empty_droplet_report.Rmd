...
processed_input_list <- map(input_meta_data_list, process_input)
unique_samples_list <- map(preprocessed_metadata_list, ~ .x |> magrittr::extract2(sample_column) |> unique())
...
empty_df <- function(input_metadata, empty_droplets_tbl) {
  ...
  joined_data <- empty_droplets_tbl |> 
    left_join(input |> dplyr::select(.cell, !!sample_column), by = '.cell')

...
# tissue_names <- sapply(1:length(params$x1), function(i) {
#   #browser()
#         return(params$x1[[i]][[i]])
# })
# 

# tissue_names <- lapply(params$x1, function(seurat_obj) {
#     unique(seurat_obj$Tissue)
# })
# tissue_names<- unlist(tissue_names)
# 
# tissue_names <- sapply(1:length(params$x1), function(i) {
...
<!-- This plot is useful for examining the relationship between the "True empty" and "Ambiguous" cells across different samples and can be used to identify patterns between mitochondrial/ ribosomal content, and the quality of the samples. -->

<!-- ```{r echo=FALSE, message=FALSE, warning=FALSE} -->
<!-- tissue_names<- unlist(tissue_names) -->
<!-- plot_data <- dplyr::tibble( -->
<!--   tissue_name = unique(tissue_names), -->
...
<!--       # tissue_names <- joined_data$Tissue -->

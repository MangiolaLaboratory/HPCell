...
#' @param input_read_RNA_assay SingleCellExperiment or Seurat object containing RNA assay data.
#' @param filter_empty_droplets Logical value indicating whether to filter the input data.
#' @param RNA_count_threshold An optional integer for the number of RNA count per cell. Default uses 100
#' @param RNA_feature_threshold An optional integer for the number of feature count. Default uses 200
#'
#' @return A tibble with columns: Cell, nFeature_RNA, nCount_RNA, empty_droplet (classification of droplets).
#'
#' @importFrom AnnotationDbi mapIds
#' @importFrom stringr str_subset
#' @importFrom dplyr left_join mutate
...
empty_droplet_threshold<- function(input_read_RNA_assay,
  ...
  result <- colSums(filtered_counts > 0 ) |> enframe(name = ".cell", value = "nFeature_RNA") |> 
    ...
    mutate(empty_droplet = nFeature_RNA < RNA_feature_threshold | nCount_RNA < RNA_count_threshold)

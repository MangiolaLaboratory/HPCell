...
tiers_dataframe <- results |>
  mutate(file_name = file_name,
         ...
         tier = case_when(cell_number < 500 ~ "tier_1",
                          cell_number >= 500 & cell_number < 1000 ~ "tier_2",
                          cell_number >= 1000 & cell_number < 10000 ~ "tier_3",
                          cell_number >= 10000 ~ "tier_4"),
...
file_list = files |> head(2600)

#batches <- split(file_list, ceiling(seq_along(file_list$file_name) / 200))

file_list |> pull(file_name) |>
# process_batch <- function(file_subset) {
#     
#     file_subset |> pull(file_name) |>
      initialise_hpc(
        gene_nomenclature = "ensembl",
        data_container_type = "anndata",
        store = "~/scratch/Census/census_reanalysis/census-run-samples/final_run/",
        tier = file_list |> pull(tier),
        computing_resources = list(
          crew_controller_slurm(
            name = "tier_1",
            script_lines = "#SBATCH --mem 5G",
...
# Process each batch
#purrr::map(batches, process_batch, .progress = TRUE)

...
#files |> saveRDS("~/scratch/Census/census_reanalysis/census-run-samples/final_run/files.rds")
files <- readRDS("~/scratch/Census/census_reanalysis/census-run-samples/final_run/files.rds")

# Run 1000 samples per run. Save log and result in the corresponding store
setwd("~/scratch/Census/run13/")

file_list = files |> slice(12001:13000)

file_list |> pull(file_name) |>
  initialise_hpc(
    gene_nomenclature = "ensembl",
    data_container_type = "anndata",
    store = "~/scratch/Census/run13/",
